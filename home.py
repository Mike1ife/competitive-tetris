...
def run_home(
    ...
    while True:
        ...
        if last_stats:
            ...
            stat_lines = [
                f"P1: {last_stats['p1_score']}pts  l={last_stats['p1_lines']}  g={last_stats.get('p1_garbage_sent', 0)}  T={p1c.get(4,0)} t={p1c.get(3,0)} d={p1c.get(2,0)} s={p1c.get(1,0)} c={last_stats.get('p1_combos', 0)}",
                f"P2: {last_stats['p2_score']}pts  l={last_stats['p2_lines']}  g={last_stats.get('p2_garbage_sent', 0)}  T={p2c.get(4,0)} t={p2c.get(3,0)} d={p2c.get(2,0)} s={p2c.get(1,0)} c={last_stats.get('p2_combos', 0)}",
            ]
